...
import (
	...
	"github.com/cjfinnell/go-service-test-patterns/mocks"
...
func TestGet(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			req := httptest.NewRequest(http.MethodGet, fmt.Sprintf("/v1/%s", testKey), nil)
...
func TestSet(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			req := httptest.NewRequest(http.MethodPost, fmt.Sprintf("/v1/%s/%s", testKey, testValue), nil)
			w := httptest.NewRecorder()
...
func TestDel(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			req := httptest.NewRequest(http.MethodDelete, fmt.Sprintf("/v1/%s", testKey), nil)
			w := httptest.NewRecorder()
